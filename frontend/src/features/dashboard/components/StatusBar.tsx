...
export default function StatusBar({
  ...
  const tr = useTr();
  const getStatusText = () => {
    if (step === "result") return tr("当前状态：已生成文稿");
    if (step === "editForm") return tr("当前状态：编辑表单中");
    if (step === "processing" || step === "extractingData") return tr("当前状态：AI 正在处理中...");
    if (fileCount === 1) return tr("当前状态：已上传1个文档");
    return tr("当前状态：已上传 %d 个文档").replace("%d", String(fileCount));
  };

  const getNextStepText = () => {
    if (step === "result") return tr("下一步：点击下载");
    if (step === "editForm") return tr("编辑完成后可导出");
    if (step === "processing" || step === "extractingData") return tr("请等待处理完成");
    if (fileCount === 0) return tr("下一步：上传原始单据");
    if (!templateSelected) return tr("下一步：选择或创建模板");
    return tr("下一步：点击开始提取数据");
  };
